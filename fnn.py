...
class KerasFNNClassifier(BaseEstimator, ClassifierMixin):

    def __init__(self, epochs=20, batch_size=32, validation_split=0.2, verbose=0):
        self.epochs = epochs
        self.batch_size = batch_size
        self.validation_split = validation_split
        self.verbose = verbose

    def _build_model(self, n_features_in):
...
for selection_name, selection_factory in selection_methods.items():
    for k in feature_counts:
        ...
        print(f"\n{'=' * 60}")
        print(f"{selection_name} - {k} FEATURES - FNN")
        print(f"{'=' * 60}")
...
print(f"\n{'=' * 60}")
print("PODSUMOWANIE - FNN")
print(f"{'=' * 60}")
print(results_df.to_string(index=False))

results_df.to_csv("wyniki_fnn.csv", index=False)